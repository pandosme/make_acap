...
EVENTS_Add_Event( const char *id, const char* name, int state ) {
	...
	ax_event_key_value_set_add_key_value( set, "topic1", "tnsaxis", ACAPID , AX_VALUE_TYPE_STRING,NULL);
	ax_event_key_value_set_add_nice_names( set, "topic1", "tnsaxis", ACAPID, ACAPNAME, NULL);
...
int
EVENTS_Init(const char* acapID, const char* acapNiceName ) {
	LOG_TRACE("%s: Entry %s %s\n",__func__,acapID,acapNiceName);
	sprintf(ACAPID,"%s",acapID);
	sprintf(ACAPNAME,"%s",acapNiceName);
	EVENTS_SUBSCRIPTIONS = cJSON_CreateArray();
