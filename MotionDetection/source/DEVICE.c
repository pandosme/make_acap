...
//	}
	...
	if(ax_parameter_get(DEVICE_parameter_handler, "root.ImageSource.I0.Sensor.AspectRatio",&value,0 )) {
		...
		cJSON_AddStringToObject(DEVICE_Container,"aspect","16:9");
	}
	...
	if(ax_parameter_get(DEVICE_parameter_handler, "root.Properties.Image.Resolution", &value, 0)) {
		...
		cJSON_AddItemToObject(DEVICE_Container,"width",cJSON_CreateNumber(width));
		cJSON_AddItemToObject(DEVICE_Container,"height",cJSON_CreateNumber(height));
		
		int a = (width*100)/height;
		if( a == 133 )
			cJSON_ReplaceItemInObject(DEVICE_Container,"aspect",cJSON_CreateString("4:3") );
		if( a == 160 )
			cJSON_ReplaceItemInObject(DEVICE_Container,"aspect",cJSON_CreateString("16:10") );
		if( a == 100 )
			cJSON_ReplaceItemInObject(DEVICE_Container,"aspect",cJSON_CreateString("1:1") );
