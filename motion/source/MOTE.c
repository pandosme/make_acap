...
/*
		if( ignoreFlag ) {
			cJSON_AddTrueToObject(boundingbox,"ignore");
		} else {
			cJSON_AddFalseToObject(boundingbox,"ignore");
		}
*/	
		if( !ignoreFlag )
...
MOTE_Subscribe( MOTE_Callback callback ) {
	...
	STATUS_SetBool("mote","subscription",0);
	...
	STATUS_SetBool("mote","subscription",1);
	return 1;
}
